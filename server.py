...
class ActionQueue:
    ...
    def enqueue(self, ship_symbol: str, tool_name: str, args: dict) -> str:
        ...
        if len(q) >= MAX_QUEUED_ACTIONS:
            return (
                f"Error: Queue full for {ship_symbol} ({MAX_QUEUED_ACTIONS} pending)."
            )
    ...
    def tool_endpoint(payload: dict = Body(default_factory=dict)):
        with state_lock:
            try:
                ...
                if ship_symbol and tool_obj.name in (
                    WAITING_TOOLS | STATE_CHANGING_TOOLS
                ):
                    ship_status = fleet.get_ship(ship_symbol)
                    if ship_status and not ship_status.is_available():
                        ...
                        if has_behavior:
                            ...
                            raise Exception(
                                f"Error: {ship_symbol} is busy ({ship_status.busy_reason}, {secs:.0f}s remaining) and has an active behavior. cancel_behavior first."
                            )
                        elif tool_obj.name in STATE_CHANGING_TOOLS:
                            result = action_queue.enqueue(
                                ship_symbol, tool_obj.name, args
                            )
